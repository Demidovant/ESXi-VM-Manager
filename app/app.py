...
def execute_operations():
    ...
    success_count = 0
    errors = []
    operation_results = {}  # Для отслеживания статусов операций
    ...
    try:
        for vm_data in vm_operations:
            ...
            for op_name in operations:
                ...
                try:
                    if operation_interrupted.is_set():
                        errors.append(f"Прервана операция '{op_name}' для ВМ {vm_name}")
    ...
    return jsonify({
        ...
        "errors": errors,
        "vmOperations": vm_operations,
        "operationResults": operation_results  # Добавляем детальные результаты
    })
